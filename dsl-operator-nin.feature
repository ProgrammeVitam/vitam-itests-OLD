...
@Dsl
@DslOperatorNin
Fonctionnalité: Test operateur NIN du DSL
  ...
  Scénario: Dsl operator $nin sur les contrats d'accès

    # Type strict_date_optional_time
    Quand j'utilise le fichier de requête suivant data/queries/select_accesscontracts_dsl_nin.json
  ...
  Scénario: Dsl operator $nin sur les unités archivistiques
    Etant donné les tests effectués sur le tenant 0
    Et les données du jeu de test du SIP nommé data/SIP_OK/ZIP/OK_ARBO_rateau_MD_complexes_match.zip

    # Type strict_date_optional_time
    Quand j'utilise le fichier de requête suivant data/queries/select_units_dsl_nin.json
